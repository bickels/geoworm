...
#%%
cdict = mpl.cm.get_cmap('RdBu')._segmentdata.copy()
cdict['red'][5] = (0.5,0.9,0.9) 
cdict['red'][6] = (0.6,0.7,0.7) 
cdict['red'][8] = (0.9,0.2,0.2)
cdict['red'][9] = (1.0,0.3,0.3)
cdict['green'][4] = (0.4,0.75,0.75)
cdict['green'][5] = (0.5,0.8,0.8)
cdict['green'][8] = (0.9,0.9,0.9)
cdict['green'][9] = (1.0,1.0,1.0)
...
#cdict['alpha'][:] = zip(np.linspace(0.0,1.0,10),np.linspace(0.0,1.0,10),np.linspace(0.0,1.0,10))

cm0 = mpl.colors.LinearSegmentedColormap('Spec',cdict)

cmap = mpl.colors.LinearSegmentedColormap.from_list('trunc', cm0(np.linspace(0.1, 0.8, 2**8)))

#cmap=cm0
#%%
ds015 = xa.open_dataset(r'.\out\days_above_0.15.nc')
ds02 = xa.open_dataset(r'.\out\days_above_0.2.nc')
...
#    ax.coastlines()
    ...
    cb = plt.colorbar(**cbpars)
    cb.set_label(r'Days of potential activity $\theta_c$=0.25 [$y^{-1}$]')
    plt.xlabel('lon')
    plt.ylabel('lat')
    plt.tight_layout()
    pdf.savefig(dpi=300)
...
#    ax = plt.axes(projection=proj)
#    ax.coastlines()
    plt.imshow(ds02.sm-ds025.sm,cmap='Spectral')
    cb = plt.colorbar(**cbpars)
...
#    ax = plt.axes(projection=proj)
#    ax.coastlines()
    plt.imshow(ds02.sm-ds015.sm,cmap='Spectral')
    cb = plt.colorbar(**cbpars)
...
with PdfPages(r'.\res\days_above_plot_ROI.pdf') as pdf:
#    ax = plt.axes(projection=ccrs.InterruptedGoodeHomolosine())
#    ax.coastlines()
#    plt.title('Global')
#    plt.pcolormesh(ds02.lon,ds02.lat,ds02.sm,cmap=cmap,vmin=dmin,vmax=dmax)
#    cb = plt.colorbar(**cbpars)
#    cb.set_label('days above 0.2 ($y^{-1}$)')
#    plt.xlabel('lon')
#    plt.ylabel('lat')
#    plt.tight_layout()
#    pdf.savefig(dpi=300)
...
#    plt.title('Global')
    ...
    plt.pcolormesh(dat.lon,dat.lat,dat.sm,cmap=cmap,vmin=dmin,vmax=dmax)
    cb = plt.colorbar(**cbpars)
    cb.set_label(r'Months of potential activity $\theta_c$=0.2 [$y^{-1}$]')
    plt.xlabel('lon')
    plt.ylabel('lat')
